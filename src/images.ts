export const apple = require('../assets/apple.png');
export const google = require('../assets/google.png');
export const logo = require('../assets/logo.png');

export const categoryIcon = require('../assets/category_icon.png');
export const categoryIconActive = require('../assets/category_icon_active.png');
export const myInfoIcon = require('../assets/myInfo_icon.png');
export const myInfoIconActive = require('../assets/myInfo_icon_active.png');
export const todoIcon = require('../assets/todo_icon.png');
export const todoIconActive = require('../assets/todo_icon_active.png');
export const chevronRightIcon = require('../assets/chevron_right_icon.png');

export const cdSampleImage = require('../assets/cd_sample.png');
export const modalBackground = require('../assets/modalBackground.png');
export const coffee = require('../assets/routine_cd/coffee.png');
export const eat = require('../assets/routine_cd/eat.png');
export const morning = require('../assets/routine_cd/morning.png');
export const read = require('../assets/routine_cd/read.png');
export const running = require('../assets/routine_cd/running.png');
export const travel = require('../assets/routine_cd/travel.png');
export const tutorialAudio = require('../assets/tutorial/tutorial_audio.png');
export const tutorialPhone = require('../assets/tutorial/tutorial_phone.png');
